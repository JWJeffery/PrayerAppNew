# RESUME_PROJECT_NOTE — Historical Archive (superseded detail, 2026-07-17)

This file holds the full, unabridged detail that was cut from `RESUME_PROJECT_NOTE.md` on
2026-07-17 to keep that file navigable. It covers:

- **Anglican/BCP Daily Office work**, 2026-07-06 through 2026-07-10 (fully DONE, per the current
  `RESUME_PROJECT_NOTE.md`'s summary — this archive holds the book-by-book/season-by-season detail
  of how each defect was found and fixed).
- **Biblical Corpus remediation, session-by-session detail**, 2026-07-10 through 2026-07-16,
  covering every individual OT and NT book's character-for-character verification (KJV, DRB,
  Rotherham, NABRE, NRSV, plus Psalms' 9 translation lanes) through to the entire corpus being
  closed. All of this work is DONE and its outcomes are also recorded, per-book, in
  `AUDIT_GOVERNANCE_LEDGER.md`'s `GREEN_NOTES` entries and the dashboard's `GREEN_SEED` — this
  archive is for recovering the narrative detail of *how* a specific past fix was made (e.g. "why
  did DRB Job's chapter numbering shift"), not for current status, which lives in the trimmed
  `RESUME_PROJECT_NOTE.md` and the ledger.

**If you're trying to resume work, read `RESUME_PROJECT_NOTE.md` first — not this file.** This
archive is reference material for recovering old detail, not a starting point.

**Format note:** this is the verbatim content originally in `RESUME_PROJECT_NOTE.md` lines 34-2433
(the "Project resume log" section through the end of the New Testament/OT remediation phase),
moved here unedited when that file was trimmed. Session headers and internal cross-references are
preserved as originally written and may reference line numbers or "earlier in this file" in ways
that now point to positions within *this* archive rather than the current resume note.
...
## HANDOFF TO NEXT SESSION — 1 Enoch exhaustive verification, written 2026-07-14 at end of a long session

**Josh's explicit instruction: the two "checked every flagged chapter" passes are NOT sufficient. He wants 1 Enoch verified exhaustively** — every one of the book's 1,051 verses individually checked against real source text, not just the ~56 verses that an imperfect scrollmapper-based diff happened to flag. This note exists because the session that did the first two passes ran out of practical budget before starting that exhaustive pass. Read this in full before touching the file.

### Why "check only the flagged differences" isn't good enough — the concrete proof

The chapter 24/25 boundary defect (fixed earlier this session — see the two 1 Enoch ledger entries above) was **invisible to the scrollmapper diff**, because scrollmapper's own data shared the exact same wrong boundary the app had. A diff between two sources that share an error shows zero difference. This is a real, demonstrated failure mode, not a hypothetical — it's exactly why a scan limited to "where the two sources disagree" cannot be trusted as a complete audit. Exhaustive verification means reading the app's actual text against a real primary source directly, chapter by chapter, not trusting any diff tool as the sole method.

### Current state of the data (confirmed as of `origin/main` commit `43ca7b1`)

- `data/bible/OT/1enoch.json`: 108 chapters, **1,051 verses** (was 1,048 before this session; the 3-verse increase is restored content in chapters 24-25, not padding).
- Fixes already applied and verified this session, all committed: chapters 5 (verses 6-9), 24-25 (full boundary rebuild), 76 (verses 11-13), 90 (verses 14, 33, 40), 103 (verse 6), 106 (verse 18).
- **None of this should be treated as "these chapters are now closed."** They were fixed because a diff happened to flag them. They still need the same exhaustive re-check as every other chapter, since the diff-based process that found them is not the standard being applied going forward.

### Sources available — what's proven to work, and their real limits

1. **Project Gutenberg #77935** — `https://www.gutenberg.org/ebooks/77935.txt.utf-8` (plain text) or `https://www.gutenberg.org/files/77935/77935-h/77935-h.htm` (HTML — got further before truncating, reached partway through chapter 60 in this session). Confirmed identical translation to the app's existing text (1917 SPCK reprint of Charles's 1912 Oxford critical edition). **Real, hard limitation: `web_fetch` truncates the output at a fixed size regardless of the `text_content_token_limit` parameter — tried up to 200,000 with no change in where it cut off.** This is not a token-budget issue on Claude's end; it's the tool. Don't waste time re-trying larger limits.

2. **CCEL (Christian Classics Ethereal Library)** — `ccel.org/c/charles/otpseudepig/enoch/ENOCH_1.HTM`, hosting Charles's *Apocrypha and Pseudepigrapha of the Old Testament* (Oxford: Clarendon Press). Josh supplied this via screenshot; the page has "Section 2," "Section 3," "Section 4," "Section 5" navigation buttons visible, strongly suggesting the **entire book is available on CCEL across 5 section pages** (Section 1 = chapters I-XXXVI per the visible page title). **This was never actually fetched by Claude this session — Josh pasted the text for chapters 61-108 manually from what appears to be this same source family.** A live web_fetch of the actual CCEL URLs (Section 1 through 5) has not been attempted and could plausibly be a cleaner, non-truncating source for the *entire* book in one go — **this is the single most promising lead for the next session, worth trying first** before falling back to Gutenberg's truncating fetch. Try `web_search` for "ccel.org charles otpseudepig enoch" to get the URL into fetchable results, then `web_fetch` each section directly.

3. **`scrollmapper/bible_databases_deuterocanonical`** (GitHub, `git clone https://github.com/scrollmapper/bible_databases_deuterocanonical.git`, file at `sources/en/1-enoch/1-enoch.json`) — same translation, pre-segmented into flat verse numbers, all 108 chapters. **Confirmed unreliable as a sole source** — it silently merges verses that should stay separate in multiple places (chapters 8, 10, 14, 16, 18, 22 this session alone). Fine as a rough triage aid to prioritize which chapters to look at first, but every verse it's used for must be independently confirmed against Gutenberg or CCEL before trusting it, and it must never be the reason a chapter is skipped.

4. **Josh can paste text directly** (proven twice this session, works well) — if automated fetching keeps failing, ask him to paste chapters from CCEL or another source the same way he did for chapters 61-108 and for 3/4 Maccabees.

### Recommended method for the exhaustive pass

1. Try CCEL's 5 section pages first (see above) — if they fetch cleanly without truncation, that's a single source covering the whole book, better than juggling Gutenberg (partial, truncates) + CCEL (currently only have Josh's pasted chapters 61-108).
2. Go chapter by chapter, 1 through 108. For each chapter: pull the app's current text, pull the real source text, compare verse-by-verse by reading, not by running a diff tool as the primary method (diffs are a useful first pass to prioritize, never a substitute for reading).
3. Apply the same standing judgment already used this session: fix genuine corruption, missing content, invented details, and clear sense-breaking errors. Do NOT force-match legitimate synonym-level translation variants or the app's already-consistent diacritic-stripping convention — those are style choices already made, not defects.
4. Batch commits by reasonable chunks (e.g. every 10-20 chapters) rather than one giant commit at the end, per this project's standing "push early" discipline — if a session runs out of budget mid-pass, whatever's been verified and fixed should already be safely committed and pushed.
...
## Session, 2026-07-14 continued — Psalms: KJV switched to 1769 KJVA, NABRE book-division headers stripped

Per Josh's explicit decisions this session:

**KJV lane switched from 1611 original spelling to 1769 KJVA**, matching the rest of the corpus (the KJVA-edition-switch decision documented in `documentation/kjva-source-witness-intake.json`). Full-column replacement from `scrollmapper/bible_databases` `formats/json/KJVA.json` (same source already used and trusted for the Rotherham lane's own source family), matched chapter:verse, all 150 psalms / 2461 verses. Re-verified at zero mismatches against source after the swap.

**NABRE book-division headers stripped** from the 5 psalms that open each of the Psalter's traditional "5 books" (1, 42, 73, 90, 107) — text like "First Book— Psalms 1–41 Psalm 1 True Happiness in God's Law" was prepended to verse 1's real content in each case. Removed the book-division+individual-psalm-title portion, preserving genuine content that follows (including legitimate internal Roman-numeral section markers like "I"/"II"/"III" that recur throughout NABRE's psalm text elsewhere, and genuine superscriptions like "A psalm of Asaph" / "A prayer of Moses, the man of God" — these stay, consistent with how such superscriptions are already treated everywhere else in this project, e.g. Psalm 3:1's "A psalm of David, when he fled from his son Absalom.").

**Psalms KJV/NABRE lanes are now fully clean and verified** — combined with the earlier-this-session confirmations for DRB, Coverdale, NRSV, Orthodox (151-155), and Rotherham, that's 7 of 9 lanes closed.

**Grail1963 — important finding, not yet actioned:** Josh uploaded a PDF titled "Complete Revised Grail Psalter" (Conception Abbey, copyright later sold to GIA) intending it as a source to check the app's `Grail1963` lane against. Direct comparison shows this is a DIFFERENT, later translation than what the app stores — the 2010 "Revised Grail Psalms" (opens "Blessed indeed is the man / who follows not the counsel of the wicked, / nor stands in the path with sinners"), not the 1963/1986 Grail Psalter the app's `Grail1963` field actually contains (opens "Happy indeed is the man / who follows not the counsel of the wicked, / nor lingers in the way of sinners / nor sits in the company of scorners"). Every verse differs in wording throughout — confirmed on Psalm 1 verse-by-verse, not just the opening line. **This PDF cannot be used to verify the existing Grail1963 lane — it would produce false "mismatches" everywhere due to being a genuinely different translation, not a corrupted copy of the same one.** Flagged for Josh: does he want (a) the existing 1963/1986 Grail1963 lane verified against some other, correct-edition source instead, or (b) this Revised Grail Psalms text added as a new, separate lane (distinct from Grail1963), or (c) something else? Not yet resolved — no changes made to the Grail1963 lane this session.

JPS1985 still in progress (source found via Sefaria's HuggingFace database export — real 1985 JPS text — but the fetch truncates partway through the file, same issue as Gutenberg with 1 Enoch; needs fetching in sections).
...
## Session, 2026-07-14 continued — Grail1963 psalms 80-150 closed via Gemini fetch. PSALMS IS FULLY COMPLETE. NO OPEN OT/NT BOOKS REMAIN.

**HANDOFF — Psalms is now completely done, all 9 lanes, all 150 psalms. This closes the entire OT+NT phase of the remediation project.**

**The Psalms 80-150 problem and its resolution:** the liturgies.net Grail Psalter page reliably truncated at Psalm 79/80 on every fetch attempt from this session's own tools, regardless of extraction method (`markdown` vs `traf`) — confirmed to be a hard byte-level cap hit during download, not a token-formatting limit, since both methods produced the identical cutoff point. Search-snippet mining and the site's individual daily-office pages were both explored as workarounds but proved impractical (search snippets don't surface content past the same cutoff; daily-office pages only contain short excerpted psalm portions mixed with canticles, not complete psalms). **Josh suggested trying a second AI agent (Gemini) to fetch the same URL — it succeeded in one shot, returning the complete, untruncated Psalms 80-150 text.** This is a good lesson for any future truncation wall: a different agent's fetch tool may simply not share the same limit.

**Content verification, Psalms 80-150:** 1185 verses checked. 81 apparent mismatches found; 15 were genuine typos/errors and fixed (80:9, 81:10, 88:10, 89:23, 98:2, 101:6, 104:14, 110:2, 112:1, 119:14, 121:7, 122:8, 128:3, 144:3, 145:14). The remaining 66 were all in the same three non-defect categories already established for chapters 1-79: the consistent de-hyphenation style convention, this session's own parsing artifacts (Latin psalm-title subtitles bleeding into verse text), and the app's own deliberate Hebrew-letter-name glosses in Psalm 119 (e.g. "*Beth (B) II" — genuinely helpful reader annotations already present in the app, not a copy error).

**Total Grail1963 fixes across the whole book, both halves combined:** 923 mechanical corpus-wide corruption instances (6 stray-slash + 905 stray-period + 12 compound/missed instances) + 23 individual content typos (8 in chapters 1-79, 15 in chapters 80-150) = 946 total defects found and fixed in this single lane.

**Governance file updated to reflect genuine full verification:** `psalms-legacy-subsystem-governance.json` schema stays v12; `Grail1963`'s `laneSourceTrust` status changed to `independently_verified_2026-07-14_all_150_psalms` with full detail on method, coverage, and fixes for both halves. Top-level `grailPolicy` and the whole-file `status` field updated to `governed_all_expected_lane_source_trust` (all 9 lanes now genuinely trusted — the audit script's own allowed-status enum has this exact value for precisely this situation). Re-ran `audit-bible-psalms-legacy-subsystem-governance.mjs` — passes.
...
# ARCHIVED FROM RESUME_PROJECT_NOTE.md ON 2026-08-19 (was 3200 lines, trimmed back down)
