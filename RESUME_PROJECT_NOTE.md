...
**Genuinely still open, deliberately deferred, each needs its own dedicated session:**
- **Sirach — 49 of 51 chapters differ from KJVA**, not a handful of local quirks. Likely reflects Sirach's real, well-documented unstable transmission history (Hebrew fragments, Old Greek, multiple historical verse-numbering systems in concurrent use), not a fixable-by-splitting versification convention like Judith's. Needs fresh triage of its own before assuming the Judith/small-batch approach will even work here — may need a fundamentally different strategy.
- **Tobit — all 13 populated chapters differ from KJVA.** Tobit survives in two substantially different ancient Greek recensions (short/GI and long/GII) that differ in actual wording and content, not just verse numbering — this may not be a "find the split point" problem at all, but a "which recension does this app's existing text actually follow, and does KJVA match it" problem. Check this before doing any mechanical work.

**Standing lesson from this whole KJV-edition-switch effort, worth internalizing for Tobit/Sirach and any future book:** chapter/verse-count mismatches against a reference source range from "nothing to do, already correct" (5 of 6 small-batch books) to "silently wrote wrong content, would have gone undetected without content-level re-verification" (Judith ch.16, Revelation 12:18 first-attempt). Never trust a count match, and never trust a script's own "changed: N, missing: 0" self-report — always independently re-verify the written output against source content at multiple points, especially near any chapter/section boundary.
