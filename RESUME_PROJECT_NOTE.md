...
- **NRSV — major finding, reframes the whole picture.** A second, trustworthy source arrived (`NRSV-CI.SQLite3`, 1989 American, no structural corruption per its own known-issues audit) and revealed three distinct problems, not one: (1) **Genesis 42:34** — confirmed isolated content substitution (duplicate of verse 20). (2) **Chapters 24-41 (661 verses) — real contamination.** Detectable British spelling and inconsistent quote-nesting throughout this specific range; 322 of 374 total book-wide mismatches (86%) concentrated here, a ~49% mismatch rate versus a low background rate everywhere else in the book. This reads as Anglicized-source contamination specific to this chapter range — not the "scattered edition-variance noise" this was framed as before the second source arrived. **Mechanism not yet understood** — a clean full-range source-swap would predict close to 100% mismatch, not 49%, so something more specific happened here and needs investigating before any fix is attempted. (3) **Chapter 50** — uses straight ASCII quotes instead of curly quotes throughout, a third, separate, undiagnosed anomaly.
...
**Immediate next steps:** investigate the ch.24-41 contamination mechanism before deciding the fix approach; diagnose ch.50's straight-quote anomaly; fix the confirmed isolated 42:34 defect; decide on the KJV/NABRE mechanical prefix/heading fixes (low-risk, ready to go, independent of the NRSV work).
