...
- **NRSV** — diffed against NRSV-ACE (Josh-supplied SQLite file, `NRSVACE.SQLite3`). Most differences are British-vs-American spelling/idiom (not a defect). **One confirmed real defect: Genesis 42:34's stored text is a near-duplicate of verse 20's text, not a translation of verse 34** — confirmed via three independent NRSV sources; a within-chapter duplicate scan across all 1,533 verses found this is isolated, not systemic. ~40-50 other differences still need adjudication against a plain 1989 NRSV, since NRSV-ACE has its own real Catholic-edition-specific revisions and isn't a perfect stand-in. Josh has a possible NRSV-CE source but is checking it for its own corruption issue before sending it.
...
**Immediate next steps:** confirmed 42:34 (via 3 independent sources, isolated per duplicate scan) is ready for a bounded fix; decide on the KJV/NABRE mechanical prefix/heading fixes (low-risk, ready to go); adjudicate remaining ~40-50 NRSV differences once a trustworthy plain-1989/NRSV-CE source is available.
