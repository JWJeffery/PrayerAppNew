...
**Current task, in progress:** working through the "small batch" (1/2 Esdras, 1/2 Maccabees, Wisdom, Prayer of Manasseh) at Judith's level of care — full chapter-by-chapter verse-count comparison, then content-level spot-checks at multiple points per differing chapter (not just start/end) before declaring a chapter's fix correct, full-book re-verification against an independently-written expected-value table before committing, per-book commit+push (not batched), resume note updated after each book. **If picking this up fresh: check which of these books already show a commit in `git log` before redoing — this note is being updated as each book completes, but verify against fresh `git log --oneline -10` first per the file's own standing critical-note rule.**
