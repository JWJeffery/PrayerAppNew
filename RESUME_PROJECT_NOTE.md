...
### One real bug found and *not* fixed — flagged plainly, not papered over

The cross-file Holy Day fix has a side effect worth being honest about: **Annunciation (fixed March 25) can coincide with Easter Day itself** in years where Easter falls on March 25 — confirmed via `_getEaster`: 2035, 2046, 2103 (3 times in 100 years, genuinely rare). Currently the fixed-date check runs before Easter's own resolution, so Annunciation "wins" that collision — which is liturgically backwards; Easter Day (a Principal Feast) should always take precedence, and Annunciation should transfer to a later date instead, the same category of problem as the Visitation's Trinity Sunday collision, but with a different BCP transfer rule (Annunciation's own rubric for falling in Holy Week/Easter Week, not yet looked up). Not fixed here — genuinely rare, tangential to today's Ordinary Time scope, and deserves its own correctly-researched fix rather than a rushed one bolted onto this already-large change. Flagged clearly rather than silently shipped wrong or silently left broken without explanation.
