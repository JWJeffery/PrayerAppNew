...
**Not touched, still open:** building St. Matthias/St. Barnabas/Independence Day (data is ready, just needs the same treatment as the Visitation); designing Thanksgiving Day's moveable-date handling. The St. Mary the Virgin / St. Michael and All Angels alternate-readings toggles are now implemented — see above, in this same entry.


## Session, 2026-07-09 continued — St. Mary the Virgin / St. Michael and All Angels alt-EP toggle: BUILT, patch delivered, apply status uncertain
...
**Apply status: RESOLVED.** The initial `git am` failure was investigated: the hash-mismatch theory (committer-date stamping) was checked first and ruled out by direct tree comparison -- Josh's actual pushed `origin/main` and the architect's working copy were byte-identical through the bracket-fix commits. **The real cause: the Holy Days lectionary-track fix (previous session entry) had been presented as a patch but never actually applied by Josh**, so the toggle feature's patch -- built on top of that fix -- had context (the Visitation rename, St. Andrew's new field, the new Collect component) that didn't exist in Josh's real repo yet. Not a data-integrity problem, just a skipped step. Fixed by rebuilding both commits (Holy Days fix, then the toggle feature) directly against Josh's actual pushed `HEAD` and delivering a corrected patch. Standing lesson: when a patch fails to apply, check whether every prior patch in the sequence was actually confirmed applied before assuming a benign explanation -- verify, don't default to the reassuring answer.

**Immediate next step for whoever resumes:** confirm Josh has pushed `origin/main` up through at least "Fix: 3 Ordinary Time Sunday citations..."; pull fresh; regenerate the alt-EP-toggle patch (`data/season/ordinary2.json`, `index.html`, `js/office-ui.js` changes -- described above, not yet confirmed landed in Josh's repo) against that real HEAD; reverify `git am` succeeds before considering this feature done.

**Still separately open from the Holy Days audit (independent of the above):** building St. Matthias, St. Barnabas, and Independence Day entries (BCP readings and both-rite Collects already sourced, above); designing Thanksgiving Day's moveable-date (4th Thursday of November) handling, which needs different schema treatment than every other fixed-date entry in this file format.
