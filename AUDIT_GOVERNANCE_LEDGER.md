...
## Judith remediation, session 2026-07-13 continued — resolves blocked NRSV status, confirms Tobit's header-pollution pattern recurs

Second deuterocanonical book in the current pass, with its own extensive prior 2026-06-28 tooling (`judith-source-address-policy.json`, `judith-bound-source-exact-collation-result.json`, `judith-text-trust-status.json`, several one-off audit/repair scripts), all read in full before touching anything. KJV, NABRE, and DRB were already marked trusted from that earlier work and from the 2026-07-12 KJVA edition-switch effort (commit `3733a42`, which itself found and fixed a genuine hidden-drift problem in this book's KJV chapter 15/16 boundary); NRSV was left explicitly blocked, matching Tobit's pattern exactly.

**KJV — re-confirmed clean, but only after real investigation, not a rushed re-flag.** A same-address comparison against KJVA's raw source initially reported 9 mismatches (15:14, 16:1-8) — the same shape of finding that would, in an earlier book, have signaled a real regression. Before treating it as one, checked `git log` for this file and found commit `3733a42`'s own message describing exactly this situation: KJVA's own official chapter/verse numbering diverges from the common numbering (shared by DRB/NABRE/NRSV and this app's grid) at this exact boundary. Confirmed by hand that the current state is the *already-fixed* state, not a reversion: address 15:14 correctly holds "Judith began this thanksgiving..." (genuinely KJVA's own verse 16:1), chapter 16 verses 1-8 are correctly offset by the same +1 relative to KJVA's own numbering, and KJVA's own single continuous verse 16:8 is correctly split across the app's 16:7/16:8. All four other translations (DRB, NABRE, NRSV) agree with the app's placement at these shared-grid addresses, confirming this is genuinely correct, not a coincidental match. **This is the second confirmed instance of a translation-specific chapter/verse-boundary divergence in this remediation effort (after Nehemiah's NABRE finding), and it very nearly got mistaken for a fresh regression before checking history first** — a real, concrete example of why Josh's standing caution about not flattening distinct versification traditions matters in practice, not just in principle.

**Rotherham — correctly absent** (never covered the Apocrypha).

**DRB — clean, zero content mismatches.** Several chapters (1, 2, 7, 8, 10, 11, 14) show verse-count differences where the app's grid has more slots than DRB's own source has content for. Checked every flagged address directly rather than assuming the already-established "DRB-only-additions" pattern applied uniformly — all confirmed already correctly blank, consistent with the well-documented Vulgate-vs-Greek recension-length difference for this book (DRB, following Jerome's Vulgate translation from a shorter Aramaic/Chaldee source, is genuinely shorter than the Greek-based tradition NRSV/NABRE/KJVA follow in several places).

**NABRE — 17 verses fixed, the same book-specific gap already found in Tobit.** This deuterocanonical book was outside the corpus-wide 37-protocanon-book header-pollution fix's scope and had never been touched by that earlier work. All 16 chapter-opening addresses plus one mid-chapter division-title address (10:11, "IV. Judith Goes out to War") checked directly against raw source before trusting the audit tool's computed values — per the lesson from Tobit's one over-stripping near-miss, every one of the 17 was individually verified as a genuine, correctly-computed strip; none showed the over-matching failure mode this time.

**NRSV — the major finding, previously blocked exactly as Tobit's was** (`"blocked_pending_approved_source_witness_or_policy"`). 150 of 340 source rows corrected against `NRSV-CI.SQLite3`. Dominated by the identical *class* of section-header-pollution defect first confirmed in Tobit — editorial subtitles ("Arphaxad Fortifies Ecbatana," "Nebuchadnezzar Issues an Ultimatum," "Entreaties for Peace," and a dozen more) baked directly into verse text. Independently re-verified as genuine pollution against multiple published NRSV sources (Bible Society UK, Biblia.com, Bible Study Tools) all showing clean text with no such headers — this is the second book confirming the pattern recurs, though it remains something to check independently in each remaining deuterocanonical book rather than assume. Also fixed one literal placeholder-string bug (11:1 held the bare string `"11"`, same class as Tobit's 1:1/2:1). **Before applying the bulk fix, scanned the entire changes list programmatically for any Tobit-9:3/9:4-style documented internal-order exception** (parenthetical embedded verse numbers, suspiciously short values on both sides) — found none this time, so no exclusions were needed.

**Diff is exactly 167 insertions/167 deletions (150 NRSV + 17 NABRE), nothing else touched.** All applicable translations re-audited after the fix: DRB/NABRE/NRSV zero mismatches; KJV's 9 tool-reported "mismatches" at the chapter 15/16 boundary are the expected, permanent, correct artifact of the already-fixed versification difference described above, not a defect.

**Registry files updated in place** (`judith-text-trust-status.json`, `judith-bound-source-exact-collation-result.json`), correction prepended rather than silently overwritten, with a full `versificationNote` documenting the KJV chapter 15/16 finding in detail — including an explicit reminder that this is a KJV-specific fact, not evidence of a Protestant-vs-Catholic split, since DRB/NABRE/NRSV all already agree with each other and with the app's shared-grid numbering at this exact boundary.

**Judith is now fully clean across all five applicable translations (Rotherham correctly absent) — zero known open defects, and the blocked NRSV status is resolved.** Moved to green on the dashboard.

**Standing lesson reinforced by this book specifically:** a tool-reported "mismatch" against a raw source is not automatically a defect, especially at a chapter/verse boundary — check git history and the actual current content by hand before assuming a regression, the same discipline already applied to genuinely new findings throughout this project. The NRSV section-header-pollution pattern is now confirmed in two of two deuterocanonical books checked so far; still not enough evidence to assume it's universal, but enough to make it the first thing to check in every remaining one.

## Esther remediation, session 2026-07-14 — plain Esther closed; Esther-with-Additions partially done, NRSV deliberately deferred rather than rushed

Third deuterocanonical/complex-canon book in this remediation pass, and the most structurally intricate so far: three related active files (`esther.json`, plain Hebrew/Masoretic 10-chapter form; `estherGK.json`, the merged Catholic/Orthodox form with lettered Additions A-F interspersed at their canonical positions; `additionstoesther.json`, the Additions presented as their own standalone book, mirroring KJVA's own division). Extensive prior tooling specific to Greek Esther already existed from 2026-06-28 (`greek-esther-source-address-policy.json`, `greek-esther-text-trust-status.json`, `greek-esther-bound-source-exact-collation-result.json`, plus a 2026-07-02 targeted repair, `deutero-02-greek-esther-tail-nrsv-repair.json`), all read in full before touching anything.

**`esther.json` (plain Hebrew Esther) — FULLY CLOSED.** KJV, Rotherham, DRB, NABRE all clean with zero mismatches. DRB's chapter 10 shows the expected 3-vs-13 verse-count difference (Vulgate's own chapter 10 continues into what other traditions treat as the start of the Additions material) — confirmed as the known pattern, not a new defect; chapters 11-16 are correctly entirely absent from this file, since that content lives in the other two files instead. NRSV: 40 of 167 verses corrected against NRSV-CI — ordinary wording differences this time, not the header-pollution pattern seen in Tobit/Judith. Spot-verified 5:9 against a dozen independently published sources before trusting the pattern.

**`estherGK.json` (Esther with Additions A-F) — KJV, NABRE, DRB confirmed clean; NRSV deliberately deferred, not attempted under time pressure.**

- **KJV** re-confirmed clean across both ordinary chapters 1-10 (against KJVA's "Esther") and all six named Additions (against KJVA's own separate "Additions to Esther" standalone book, chapters 10-16) — this had already been fixed in the 2026-07-12 KJVA edition-switch effort (commit `94e8347`, 267 verses, zero mismatches at the time), independently re-verified here rather than trusted on the old record alone. **A real methodology catch happened along the way, worth recording precisely:** the first verification pass for Addition D reused the same source-chapter offset already established for DRB (+3, since DRB's own Vulgate chapter 15 has three preceding "Mordecai/transition" verses before the "third day" scene that Addition D begins with) — applied to KJV, this produced 16 false "mismatches." Investigating rather than trusting the false alarm revealed that KJVA's own separate "Additions to Esther" book does *not* have that same three-verse preamble at its own chapter 15; a zero-offset mapping matched every verse exactly. **The app's underlying data was already correct the whole time — the bug was in the verification method itself**, from assuming a mapping solved for one translation (DRB) would transfer directly to a different translation (KJV) without checking that translation's own actual structure first. This is a concrete, first-hand instance of exactly the mistake Josh's standing caution warns against, caught before it could produce a false "fix."

- **NABRE** — 19 verses fixed, the same ordinary-chapter (1-10) header-pollution bug already found in Tobit and Judith; this deuterocanonical file was likewise outside the corpus-wide protocanon fix's scope. The six named Additions correctly have no NABRE lane at all, matching the pre-existing policy exactly: the bound NABRE source only ever covered ordinary Esther 1-10, so there is nothing to populate for the Additions — confirmed as a genuine absence, not a gap needing a fix.

- **DRB** — re-confirmed clean across both ordinary chapters and all six named Additions, using the exact segment mapping already carefully worked out and recorded in `greek-esther-source-address-policy.json` (Addition A: source ch.11 v.2-12 + ch.12 v.1-6; B: ch.13 v.1-7; C: ch.13 v.8-18 + ch.14 v.1-19; D: ch.15 v.4-19; E: ch.16 v.1-24; F: ch.10 v.4-13 + ch.11 v.1) — zero mismatches across all of it, confirming that earlier careful mapping work has held up under independent re-derivation.

- **NRSV — investigated thoroughly, deliberately NOT fixed this session.** NRSV-CI's own internal structure for "Additions to Esther" (book_number 192) turns out to use a *third* distinct convention for where the Additions sit relative to the ordinary chapters — different again from both DRB's (separate appended chapters 11-16) and KJVA's (a fully separate standalone book with its own 1-16 chapter numbering). NRSV-CI instead interleaves each Addition's verses directly into the *same* chapter number as its narrative insertion point, using continuous verse numbering. Confirmed by direct inspection of the raw verse text (not just verse counts) that Additions A, B, C, E, and F all cleanly append to the end of their host chapter — e.g. chapter 4's 47 verses are exactly 17 ordinary verses (matching plain Esther's own chapter 4) plus Addition C's full 30 verses appended at 18-47, content-confirmed by reading the actual verse text at the boundary. **Addition D does not cleanly append.** Direct inspection of NRSV-CI's chapter 5 showed it opens with 16 verses of Addition D's own expanded "third day" narrative, and only transitions to material matching ordinary Esther's chapter 5 (from its own verse 3 onward) partway through — meaning NRSV's chapter 5 *replaces* the Hebrew text's brief two-verse opening with Addition D's much longer parallel retelling of the identical scene, rather than presenting both versions back to back the way a simple append would. Correctly reconciling this requires careful, dedicated verse-by-verse work confirming exactly where the replacement boundary falls and how the surrounding ordinary verses renumber as a result — not a mechanical offset formula, which is exactly the kind of shortcut that produces flattening errors. Given the demonstrated real risk of getting this wrong under time pressure (as very nearly happened with the Addition D *KJV* false-mismatch just above, on a considerably simpler case), this was deliberately left for a focused follow-up rather than pushed through this session.

**`additionstoesther.json` (the standalone Additions book) — not yet started at all this session.** Needs its own full five-lane audit: KJV and NRSV against KJVA's and NRSV-CI's respective standalone "Additions to Esther" books, DRB against `esther.usfm`'s chapters 11-16, and NABRE — very likely absent, matching the same policy already established and reconfirmed for `estherGK.json`'s Additions, but not yet independently checked for this specific file.

**Diffs this session:** `esther.json` exactly 40/40 lines; `estherGK.json` exactly 19/19 lines. Both independently verified after the fix. Esther as a whole is NOT closed and must not be marked green on the dashboard until `estherGK.json`'s NRSV lane (Addition D specifically) and all of `additionstoesther.json` are done.

**Standing lesson reinforced concretely twice in one book:** a mapping that correctly reconciles one translation's versification against a shared structure (DRB's Addition-lane mapping, painstakingly worked out on 2026-06-28) cannot be assumed to transfer to a different translation's own internal structure without independent verification — confirmed true for KJV (where reusing DRB's offset produced a brief false alarm, quickly resolved) and decisively true for NRSV (where the actual content itself, not just the numbering, genuinely differs in how Addition D relates to its surrounding chapter). This is precisely the discipline Josh has asked this project to maintain, now demonstrated in practice rather than only in principle.

## Esther remediation, session 2026-07-14 continued — ordinary-chapter NRSV fixed; Additions' NRSV mapping proves more complex than the previous session's initial assessment

Continuing directly from the prior session's Esther work. Fixed `estherGK.json`'s ordinary-chapter (1-10) NRSV lane cleanly: 40 verses corrected by copying the already-verified-correct text directly from plain `esther.json` (the same underlying Hebrew narrative, sourced from the same NRSV-CI "Esther" book) rather than re-deriving through NRSV-CI's more confusing "Additions to Esther" continuous-numbering structure. Confirmed these were the exact same 40 diffs plain `esther.json` itself needed before its own fix, applied identically — zero remaining ordinary-chapter mismatches after.

**Investigating the Additions' NRSV mapping surfaced real, demonstrated complexity — caught before any fix was written, not after.** The previous session's entry assumed Additions A, B, C, E, and F would each cleanly append after their host chapter's ordinary content (based on Addition C's clean 17+30=47 arithmetic and the fact that Addition D alone was known to interleave). That assumption turned out to be wrong for at least two of the five:

- **Addition A is a prologue, not a trailing append.** Reading NRSV-CI's actual chapter 1 verse text (not just counts) showed verses 1-17 are Mordecai's dream and the eunuchs' conspiracy — Addition A's own content — with ordinary chapter 1 (the king's banquet, Vashti's feast) following at verses 18-39. A naive "ordinary content first, addition appended after" mapping, if applied here, would have swapped Addition A's content with the ordinary chapter's content entirely — a severe, first-hand demonstration of exactly the flattening risk this project has been repeatedly cautioned about.
- **Addition B is inserted mid-chapter, not appended at the chapter's end.** NRSV-CI's chapter 3: verses 1-13 are ordinary narrative, verses 14-20 are Addition B's decree text (embedded mid-chapter, matching the narrative point where Haman's decree is issued), and verses 21-22 return to two further ordinary verses describing the decree's publication.
- **Addition E (chapter 8) shows a similarly interleaved shape** on inspection of the actual verse text — ordinary narrative, then the king's reversing letter, then a short ordinary coda — that does not cleanly match the verse-count boundary initially assumed. Not yet fully resolved.
- **Addition C (chapter 4) does appear to be a genuine clean end-of-chapter append**, content-confirmed directly: verses 1-17 match plain Esther's own chapter 4 exactly, verses 18-47 are Mordecai's and then Esther's prayers (Addition C's own content).
- **Addition F (chapter 10) has not yet been content-confirmed**, only checked by verse-count arithmetic so far, which is exactly the kind of check that just failed for Additions A and B.

**This is the second instance within the Esther remediation where a mapping assumption, if trusted on arithmetic alone, would have produced genuinely wrong content** — the first being the KJV Addition-D false-mismatch from reusing DRB's offset (caught and resolved as a verification-method error, not a real defect); this one is more serious, since it was caught *before* writing anything, at the investigation stage, rather than after a fix was already applied. Given the real, now twice-demonstrated risk of exactly this failure mode in this specific book, completing the Additions' NRSV mapping correctly requires individual, unhurried, content-verified reconciliation for each Addition — not a formula reused across them, and not something to complete under time pressure in the same session it was discovered. Deferred to dedicated follow-up work.

**Diff this session:** exactly 40/40 lines (ordinary chapters 1-10 only). No Addition-lane NRSV text was touched or guessed at.

**Esther remains NOT closed.** Current state: `esther.json` fully closed; `estherGK.json`'s KJV/NABRE/DRB fully closed and its ordinary-chapter NRSV now also fixed, but all six Additions' NRSV lane still needs individual verification (Addition C looks safe pending final content confirmation; A, B, D confirmed to need non-trivial, non-formulaic handling; E and F not yet fully resolved); `additionstoesther.json` not yet started. Do not mark Esther green on the dashboard until all of this is complete.

## Esther remediation, session 2026-07-14 continued — ESTHER FULLY CLOSED, all three files

Completing Esther, with explicit confirmation from Josh that no time pressure applied — each remaining Addition got the careful, individual verification the earlier sessions had deliberately deferred rather than a rushed formulaic pass.

**`estherGK.json`'s six Additions, each independently content-verified against NRSV-CI's raw text:**

- **Addition A** (chapter 1 host): NRSV-CI's own database row 1 is blank (markup tags only, no real text) — real content begins at row 2. Source row 12 combines two sentences ("Mordecai saw in this dream what God had determined to do..." and "Now Mordecai took his rest in the courtyard with Gabatha and Tharra...") that the app's own existing structure correctly splits into two separate verses (11 and 12) — preserved that split point exactly rather than disturbing it, supplying the correct wording for each half from the source. Confirmed against plain `esther.json`'s own (already-verified) chapter 1 that source row 18 is genuinely the start of ordinary chapter 1, not additional Addition A material — ruling out a same-length assumption that would have extended Addition A one verse too far.
- **Addition B** (chapter 3 host): confirmed genuinely inserted mid-chapter, not appended at the end — source rows 1-13 are ordinary narrative, 14-20 are Addition B's own decree text, and 21-22 are two further ordinary verses describing the decree's publication. The app's prior (unverified) text had paraphrased and re-split the row 18/19 sentence boundary differently than the source itself; replaced with the source's own exact wording and boundary, consistent with how this project has handled every other book's wording fixes.
- **Addition C** (chapter 4 host): confirmed as a genuine clean end-of-chapter append, source rows 18-47. A real, previously-unnoticed defect surfaced here: the app's entire existing text for this Addition used archaic "thou/thee/thy" pronouns throughout, inconsistent with the modern-English NRSV style used everywhere else in this entire project, and never actually verified against a real source per the pre-existing trust status file's own honest "unverified" framing. All 23 verses needing the pronoun modernization were corrected.
- **Addition D** (chapter 5 host): source rows 1-16 directly, no offset — already established in the immediately preceding session; re-confirmed here as part of the comprehensive pass, 7 wording corrections applied.
- **Addition E** (chapter 8 host): confirmed as source rows 13-36, not 13-37 as a naive verse-count difference would suggest. Source row 37 ("Let copies of the decree be posted conspicuously in all the kingdom, and let all the Jews be ready...") has no equivalent content anywhere in the app's existing 24-verse Addition E structure. Rather than assume this was a gap needing a fix, checked DRB's own independently-collated chapter 16 (also exactly 24 verses, under the long-established `greek-esther-source-address-policy.json` mapping) and confirmed DRB's own final verse likewise has no corresponding content for this sentence. This is a genuine, minor granularity difference specific to NRSV's own textual witness, not represented in the shared 24-verse grid every other translation in this book already uses consistently — documented as such rather than inventing a 25th verse or awkwardly merging two unrelated sentences into one to force a fit.
- **Addition F** (chapter 10 host): confirmed as source rows 4-14, a clean end-of-chapter append — this one matched the original arithmetic-based expectation, but was still content-verified line by line rather than assumed, given how badly that assumption had failed for Additions A and B earlier in this same investigation.

**Independently cross-checked the entire reconstruction against a completely different published source:** Bible Study Tools' "Additions to Esther" NRSA presentation, which groups the identical underlying material under an entirely different chapter-numbering scheme (continuous numbering through what would be "chapter 4" extending to verse 63, rather than NRSV-CI's own chapter-per-host-chapter grouping). Zero contradictions found in either content or verse ordering between the two independently-structured presentations — strong corroborating evidence that the mapping work above is sound.

**`additionstoesther.json` (the standalone Additions book):** confirmed to have identical structure and identical underlying content to `estherGK.json`'s own six Additions (same lettering, same verse counts: A=17, B=7, C=30, D=16, E=24, F=11). KJV and DRB verified clean using the exact same source-chapter mappings already established and independently confirmed for `estherGK.json`'s Additions. NABRE correctly absent throughout (zero populated rows), consistent with the long-established policy that the bound NABRE source only ever covered ordinary Esther 1-10. NRSV: 72 of 105 verses corrected by copying the now-fully-verified text directly across from `estherGK.json`'s own Additions, verse for verse by letter and number — first spot-confirming that both files genuinely held the same underlying pre-fix (unverified) text, which they did, making the direct copy safe and appropriate rather than requiring independent re-derivation.

**ESTHER IS NOW FULLY CLOSED — all three active files (`esther.json`, `estherGK.json`, `additionstoesther.json`) are clean across every applicable translation lane, zero known open defects anywhere in this book.** Moved to green on the dashboard.

**Standing lesson from this book as a whole, now demonstrated concretely across two full sessions:** a mapping that works for one translation's own versification, or an arithmetic verse-count difference that looks clean on the surface, cannot be trusted without reading the actual content at the boundary in question. This book alone produced three near-misses that would have written wrong content had they not been caught by direct verification: the KJV Addition-D false-mismatch (a verification-method error, not a real defect), Addition A's reversed prologue-vs-append structure, and Addition B's mid-chapter insertion point. Every one was caught before being written to the file, by reading the actual text rather than trusting the numbers. This is the discipline this whole remediation effort has been built around, and Esther is the clearest demonstration yet of why it matters in practice, not just in principle.

## 1 Maccabees and 2 Maccabees remediation, session 2026-07-14 continued — both fully closed, NRSV header-pollution pattern now confirmed in four consecutive deuterocanonical books

Unlike Tobit, Judith, and Esther, neither Maccabees book had dedicated prior tooling or registry files. Both were simply named on the 2026-07-12 KJVA-edition-switch supersession note's "likely stale, not yet individually confirmed" list — re-verified here with the current five-lane tooling rather than assumed stale or assumed fine.

**1 Maccabees.** KJV clean against KJVA (no fix needed — this book's KJV column was apparently already accurate). Rotherham correctly absent. DRB clean, zero chapter-count differences. NABRE: 17 verses fixed, the familiar chapter-heading-pollution bug (16 chapter openers plus one mid-chapter division title, 9:23, "III. Leadership of Jonathan") — every address checked directly against raw source before trusting the audit tool's computed values; no over-stripping found. NRSV: 320 of 924 verses corrected against NRSV-CI — **the third consecutive deuterocanonical book to confirm the section-header-pollution pattern** first found in Tobit, independently re-verified via multiple published sources (Bible Study Tools' NRSA and RSVA presentations, Biblia.com). Also fixed the same literal placeholder-string bug already seen in Tobit and Judith (1:1 held the bare string `"1"`). Scanned the full changes list for order-exception anomalies before applying in bulk — none found; spread across all 16 chapters, no contamination signature. Diff: 338/337 lines (a harmless 1-line JSON-formatting asymmetry from the two-pass fix process, content-verified as zero mismatches across all three corrected lanes).

**2 Maccabees.** Same clean picture across KJV, Rotherham (absent), DRB. NABRE: 19 verses fixed. One address (8:11) was notably *not* a header-stripping case at all — the raw source has no chapter marker or title material there whatsoever; the actual defect was a minor stray-space-around-an-em-dash formatting issue ("talent —little" vs. the source's "talent—little"), a useful reminder that this audit lane occasionally surfaces small formatting bugs alongside its more common header-pollution catches, and that each flagged address still deserves individual inspection rather than a blanket assumption about what kind of fix it needs. NRSV: 201 of 555 verses corrected — **the fourth consecutive deuterocanonical book to confirm the pattern**, independently re-verified via five separate published sources (Biblia.com, Bible Society UK, Bible Study Tools, bible-history.com, biblehub.com's NRSVCE presentation). Diff: exactly 220/220 lines, nothing else touched.

**Both books are now fully clean across all five applicable translations — zero known open defects.** Moved to green on the dashboard.

**Standing observation, now with four data points:** the NRSV section-header-pollution pattern has been confirmed in every deuterocanonical book checked so far (Tobit, Judith, 1 Maccabees, 2 Maccabees) — four for four. This is now close enough to a settled expectation that it should be the first thing checked in any remaining deuterocanonical book's NRSV column, though the discipline of confirming it independently each time (rather than assuming it sight-unseen) remains worth keeping, consistent with how this project has generally treated recurring patterns.

## Job remediation, session 2026-07-13 — fully closed; new defect class confirmed (two independent, unrelated chapter-boundary versification shifts in the same section, one in DRB and one in NABRE)

KJV and Rotherham clean, zero mismatches. NRSV: 180 of 1070 verses corrected, spread evenly across the book (no contamination-range or header-pollution signature — expected, since Job is protocanonical and that pattern has so far only been confirmed in deuterocanonical books). One correction (41:11) was a substantial wording change rather than a minor variant, so it was independently corroborated against three further sources beyond NRSV-CI before being trusted — confirmed the app's prior text was a leftover *RSV* rendering that had never been updated to NRSV's own deliberately different 1989 wording, not a corruption.

**New governance-relevant defect class, generalizing the README's existing item 6 (NABRE chapter-boundary shifts, first found in Nehemiah): a single book can have *multiple, independent* chapter-boundary versification shifts across *different* translations, at different split points, in the same passage.** Job's Behemoth/Leviathan section (common Job 39:31–41:34) has both: DRB/Vulgate's own chapter break falls where common numbering is 9 verses further along by the end of the section (Vulgate ch.40 absorbs common 40:6 through 41:9), and NABRE's own chapter break falls 8 verses later than common numbering in the opposite direction of drift (NABRE ch.40 absorbs common 41:1-8). Both were confirmed the same way: by reading actual verse content at the suspected boundary (the "Canst thou draw out leviathan"/Leviathan-introduction line) and finding it sitting at a different chapter/verse address than the common grid's own KJV column already correctly has it — not by trusting either audit tool's chapter-count-diff output alone, which flags the *symptom* (chapter counts not matching 1:1) but doesn't diagnose *where* the true content boundary actually is. Standing rule going forward: when `audit_drb`'s chapter-count diff or any NABRE mismatch cluster appears in a book's back half (matching the pattern of "many affected addresses, clustered together, not evenly spread"), check for a chapter-boundary shift specifically — read the actual text at the suspected transition point against the app's own already-verified KJV column at that address — before assuming either a generic content-mismatch bug or a DRB source-shape absence (per the existing registry).

**A second finding, orthogonal to the versification shift: `translationOverlays` entries are not universally consumed.** `js/scripture-resolver.js` only ever reads `translationOverlays.Rotherham` — an NABRE overlay block found in `job.json` (six entries in chapter 41, coincidentally holding what turned out to be the *correct* final values for those verses) was confirmed dead, non-functional data via direct code inspection, not a live partial fix that had been interrupted. Removed rather than left in place, since stale-but-plausible-looking overlay data for an unconsumed translation key is exactly the kind of landmine that could mislead a future audit into believing a fix was already (partially) applied. **Standing rule:** before trusting any `translationOverlays` entry as evidence of prior work, confirm via `grep -n "translationOverlays" js/` which translation keys the rendering code actually reads — do not assume every overlay key is live just because the mechanism exists for `Rotherham`.

Also fixed: one ordinary NABRE stray-space instance (34:16), and one NABRE chapter-heading-pollution instance at the NABRE chapter 41's own true starting verse (common address 41:9), using the established stripping pattern.

**Job is now fully clean across all five applicable translations — zero known open defects.** Moved to green on the dashboard. `audit_drb`'s chapter-count-diff flags at 16, 39, 40, 41, 42 and `audit_nabre`'s same-address mismatches throughout ch.41 will continue to show after this fix — both are expected, permanent artifacts of the (correctly resolved) versification differences, not indicators of remaining work. Chapter 42's DRB 16-vs-17 verse-count difference is a separate, already-governed "blank by design" case (no Vulgate equivalent to common 42:17), not part of this shift.
...
## Song of Songs remediation, session 2026-07-13 continued — fully closed, fourth instance of the chapter-boundary-shift defect class, two independent shifts at two different boundaries (DRB and NABRE), plus one shift spanning an entire chapter from verse 1

KJV, Rotherham: zero mismatches. DRB: two separate shifts. First, this source's chapter 1 has no title verse for common 1:1 — its own numbering begins at what's common 1:2, a genuine structural absence (not a bug), with the whole chapter (1:2-17) shifted by one relative to common numbering. Caught only by checking the entire chapter against KJV rather than just the tail where the count diff appeared — an important reinforcement of the standing rule from the Job entry, now sharpened: **check the whole chapter outward from a found shift, not just the verses nearest the reported diff.** Second, an independent shift at common 5/6 (common 6:1 = this source's own chapter 5's extra verse; 6:2-13 = its own chapter 6:1-12). NABRE: a third, separate shift at a different boundary (common 6/7) — common 6:13 = NABRE's own chapter 7 v.1; common 7:1-13 = its own 7:2-14. Also fixed a stray-space bug (1:5), a genuine content-loss defect (5:1, missing an entire first sentence), and a missing-space formatting artifact where a speaker-label letter had run together with the next word (7:9) — this book's NABRE text retains internal speaker-part markers (W/M/D?) as literal content throughout, consistent with existing convention, so those were preserved rather than stripped. NRSV: 21 of 117 verses corrected against NRSV-CI, confirmed no chapter-count divergence for NRSV specifically, ordinary wording differences throughout.
...
## Hosea remediation, session 2026-07-13 continued — fully closed, first minor prophet; two more independent chapter-boundary shifts (DRB at 13/14, NABRE at 1/2), plus a new sub-pattern: a within-chapter verse-split resolved by merging rather than searching for a nonexistent boundary crossing

KJV, Rotherham: zero mismatches. DRB source confirmed filed under Vulgate name `osee.usfm`. Two DRB findings: a standard chapter-boundary shift at common 13/14 (common 13:16 = source's own 14:1), and a genuinely different case — within common chapter 2, the source splits into two verses what KJV keeps as one (2:23), confirmed by KJV's own wording visibly combining both halves. **New standing distinction worth keeping: not every chapter-count diff is a boundary crossing needing a rebuild of everything downstream — some are simple within-chapter splits, resolved by merging the source's extra verse into the single common slot (matching how KJV itself already handles the same text) rather than hunting for a shift boundary that isn't there.** NABRE: an independent shift at common 1/2 (common 1:10-11 = source's own 2:1-2; common 2:1-23 = source's own 2:3-25), confirmed content-for-content against KJV. Also fixed a genuine content-loss defect (7:1, missing the verse's entire opening clause) and four ordinary LORD-casing/stray-space instances.

NRSV: 40 of 197 verses corrected against NRSV-CI, zero chapter-count divergence confirmed first. One change (12:10) independently corroborated given its substantial difference from most other translations — confirmed via a published translators' handbook discussing NRSV's known, deliberate departure at this exact verse (ambiguous Hebrew root supporting either "parable" or "destroy," NRSV chose the latter).
...
## 1 Enoch remediation — session 2026-07-14, real work begun, major structural finding

**Source now confirmed and fetchable.** `sacred-texts.com` (the source Lucy's void 2026-07-04 certification cites) blocks automated fetching outright. Two working alternatives found and cross-verified against each other and against sacred-texts's own search-snippet text:
1. **Project Gutenberg #77935** (`gutenberg.org/ebooks/77935.txt.utf-8`) — confirmed to be the exact same translation (1917 SPCK reprint of Charles's 1912 Oxford critical edition, with the publisher's own note thanking Oxford UP for permission to reprint that translation). Fetched chapters I–LIII of CVIII directly; the web_fetch tool truncates at a fixed point regardless of token-limit parameter, so the remaining chapters (LIV–CVIII) are not yet fetched this way.
2. **`scrollmapper/bible_databases_deuterocanonical`** (GitHub, clonable directly) — same R.H. Charles translation, already verse-segmented into flat integers, 108 chapters / 1,048 verses (matching the app's own row count exactly at the structural level). Spot-verified against Gutenberg's text at chapter 1 — word-for-word identical apart from apparatus brackets.

**Governance note, worth restating plainly:** the existing `broader-canon-1enoch-text-trust-and-app-render-2026-07-04.json` certification is inside Lucy's void window (one day before her 2026-07-05 dismissal) and used a fuzzy scored-matching methodology (documented in its own companion collation file), not character-for-character verification. Per standing governance, treated as untrusted and independently re-derived from scratch.

**Major finding: this is a structurally harder book than any other tackled in this remediation effort.** A first full-book comparison against scrollmapper's data found only 343/1021 exact verse matches (34%) -- but investigating individual "mismatches" revealed **scrollmapper's own JSON has real defects of its own** (it silently merges what should be separate verses in numerous places -- confirmed directly against Gutenberg ground truth for chapters 8, 10, 14, 16, 18, 22, where the *app's* existing verse boundaries turned out to already be correct and it was scrollmapper's data that was wrong). This means scrollmapper cannot be trusted as ground truth on its own -- every flagged difference has to be checked against the actual Gutenberg/Charles text before concluding which side is right, exactly the discipline this whole project has already learned the hard way with other sources.

**Real, confirmed defects found and fixed this session (verified directly against Gutenberg, not just against scrollmapper):**

1. **Chapter 5, verses 6-9 — real content loss and verse-shuffling.** Charles's actual text uses lettered sub-verses in this passage (6a-j, interleaved in the print order with 7a-c, due to Charles's own documented textual-critical dislocation). The app's flat-numbered verse 6 had dropped an entire passage ("...all the elect shall rejoice, and there shall be forgiveness of sins, and every mercy and peace and forbearance: there shall be salvation unto them, a goodly light. And for all of you sinners there shall be no salvation, but on you all shall abide a curse.") entirely; verse 7 had verse 8's content wrongly appended to it; verses 8 and 9 were actually the two halves of Charles's single verse 9, with Charles's real verse 8 ("And then there shall be bestowed upon the elect wisdom...") missing everywhere. Rebuilt all four verses (6-9) from Charles's actual text, grouping each verse's lettered fragments together by their own verse number rather than the interleaved print order -- the same category of editorial decision already used elsewhere in this project for split/dislocated verses (e.g. Hosea 2:23's merge).

2. **Chapters 24/25 — a real chapter-boundary shift, not just a verse issue.** The app's chapter 24 stopped three verses early (missing "amongst them was a tree..." through "...was their leader"); chapter 25 had absorbed those three verses at its start, plus its own first four, but as a result had genuinely **lost its true final three verses entirely** ("Its fruit shall be for food to the elect..." / "Then shall they rejoice with joy and be glad..." / "Then blessed I the God of Glory...") -- not shifted, actually missing from the book. Confirmed chapter 26 already starts correctly at Charles's true XXVI.1, so the fix is fully contained to the 24/25 boundary. Rebuilt: chapter 24 now has 6 verses (was 3), chapter 25 now has 7 verses with its correct content (was 7 verses, but the wrong 7). Book's total verse count is now 1,051 (was 1,048) -- a real increase, since real content had to be restored, not just relabeled.

**Scope of what's left, characterized honestly:** a systematic case-insensitive, diacritic-normalized comparison against scrollmapper found roughly 56 verses across ~26 chapters showing genuine severity (not just spelling/accent noise), split roughly evenly between chapters already verified against Gutenberg (12 chapters, most now investigated and several confirmed to be scrollmapper's own errors rather than app defects) and chapters beyond the current Gutenberg fetch (14 chapters: 59, 60, 61, 67, 69, 75, 76, 81, 89, 90, 91, 100, 103, 106 -- several of these, especially 89-91, are in the Animal Apocalypse / Apocalypse of Weeks sections with known textual dislocations similar to chapter 5's, so real care will be needed there too). The remaining ~509 "minor" differences found in the initial full-book pass are essentially all diacritic-stripping (the app's existing, consistent convention of simplifying Ethiopic/Aramaic name spellings -- e.g. `Semjaza` vs `Semjâzâ`) and are not defects.
...
## Governance note, 2026-07-14 end of session — 1 Enoch requires exhaustive verification, not diff-flagged spot-checks

**Josh's explicit ruling: the two passes completed this session (checking chapters flagged by a scrollmapper-based diff, across the whole book) do not meet the bar for closing this book.** He wants every one of 1,051 verses individually verified against real source text.

**The concrete reason this standard is correct, not just cautious:** the chapter 24/25 boundary defect fixed earlier this session was invisible to the diff-based method, because the comparison source (scrollmapper) happened to share the exact same wrong chapter boundary the app had. A diff between two independently-flawed sources that agree with each other on a specific error shows zero difference at that point. This is a proven failure mode from this session, not a theoretical concern — any future audit of this book that relies on diffing against a single secondary source, however good, inherits this same blind spot.

**Standing rule for 1 Enoch going forward, per this session's finding:** verification means reading the app's text against genuine primary source text (Gutenberg or CCEL, both confirmed to carry the correct R.H. Charles translation) directly, chapter by chapter. A diff tool may be used to prioritize or flag candidates, but is never sufficient on its own and must not be the basis for concluding a chapter is clean.
...
## SESSION 2026-08-18 continued -- Coptic Agpeya: Morning Office built and wired (first checkpoint)

First real content for the Coptic Agpeya rebuild, replacing the deleted Ethiopian Sa'atat. Source:
De Lacy O'Leary, *The Daily Office and Theotokia of the Coptic Church* (London: Simpkin, Marshall,
Hamilton, Kent & Co., 1911), public domain, both halves uploaded to Google Drive by Josh and read
directly via the Google Drive connector (no fetch-tool wall this time, unlike Tizaz/Fetha Nagast).

**Content built -- the full Morning Office, O'Leary's own items (1)-(12), pp. 87-97:**
- `components/coptic.json` rebuilt: 21 entries. 19 new components transcribed from O'Leary (Prayer
  of Thanksgiving, the Invitatory, the "We glorify the Trinity" hail-doxology, "O True Light," all
  nine saint doxologies (Virgin, Angels, Apostles, Martyrs, Antonius, Paul, Macarius, Cross-bearers,
  Patriarchs/Prophets), the Angelic Song, the (Coptic, non-Chalcedonian) Trisagion, the Creed without
  filioque, the 41-fold Kyrie, and the closing Metremhe prayer), plus the shared `cop-closing-formula`
  (the "Prayer recited after each Office" documented as common to all hours, for reuse when later
  hours are built). Every entry carries a `meta.source` citation to O'Leary with page numbers.
- **Confirmed content-verified, not by formula:** O'Leary never gives his own Psalm or Gospel
  translations -- only citations (e.g. "Psalm 50 (51). Morning Psalms 1, 2, 3, 4, 5, 6, 10 (11),
  11 (12), 12 (13), 14 (15), 17 (18)," and a footnoted "Lesson, Eph. iv. 1-6"). Confirmed this
  pattern holds throughout the whole book before designing the build around it -- Psalms and the
  Ephesians lesson are resolved from this app's own corpus (now correctly Hebrew-numbered, per the
  earlier session's DRB fix) via the rubric's `psalms`/`lesson` fields, not transcribed from O'Leary.
- `components/traditions/coptic/rubrics.json` created: the `coptic-morning-office` rubric, 22-item
  sequence, `psalms` field (fixed Psalm 51 + the 11-psalm morning set) and `lesson` field
  (Ephesians 4:1-6).
- **Two pre-existing unsourced stub components found and handled:** `cop-agpeya-opening` (used by
  a BCP-office toggle, cross-tradition borrowing feature) had text that matched nothing in O'Leary
  -- corrected to his real Morning Office invitatory wording. `cop-theotokion` (same toggle) is
  still unsourced -- flagged in its own `meta.status` rather than guessed at, deferred to Phase 2
  when the Theotokia gets built properly.

**Code wiring, verified with `node --check` after every edit:**
- `hydrateForCopticAgpeya()` and `renderCopticAgpeya()` added to `js/office-ui.js`, modeled on the
  East Syriac renderer (the simplest existing pattern). `appData.copticRubrics` mirrors
  `appData.eastSyriacRubrics`.
- `renderOffice()` dispatcher, `selectMode()` (new `coptic-agpeya` branch + `copSettings` panel
  variable), `UNIVERSAL_OFFICE_TRADITION_MODE_MAP`, and `resolveEntryTraditionRoute` all reconnect
  `oriental-orthodox` to the new `coptic-agpeya` mode (previously fell through to nowhere after the
  Sa'atat deletion).
- `OFFICE_MODE_HEADER_LABELS` and `BOOK_OF_NEEDS_MODE_CONTEXTS` given `coptic-agpeya` entries.
- **Deliberately did NOT add a `SHARED_OFFICE_NAVIGATOR_CONFIGS` entry for Coptic.** That
  abstraction builds a multi-option hour-picker UI; with only one hour built so far, a picker with
  nothing real to switch between would misrepresent the app's actual state. Built a plain,
  honest date-only navigator (`#coptic-settings` drawer: Prev/Today/Next using the existing shared
  `changeDate`/`resetDate` functions) instead. Revisit once more hours exist.
- `index.html`: new `#coptic-settings` drawer (replacing the placeholder comment left by the
  Sa'atat deletion); div-tag balance confirmed unchanged through each edit (242 -> 247 as the
  drawer content was added, verified balanced at each step).
- **Real bug found and fixed while wiring, not just new code:** `updateSeasonalTheme('gold')` --
  the accent color intended for the Coptic office -- matched no case in that function and silently
  fell back to the default green. Added a real `'gold'` case (`#b8860b`).
- **Stale UI text found and fixed:** the tradition-entry screen's Oriental Orthodoxy card still
  described "Ethiopian Sa'atat Book of Hours" after the deletion -- updated to describe the Coptic
  Agpeya.

**Final verification:** full-repository sweep confirms zero remaining `ethiopian-saatat` references
anywhere. `node --check` clean on `js/office-ui.js`. Both new JSON files parse. `index.html` div-tag
balance confirmed (247/247).

**Explicitly not done yet, next checkpoint:** the remaining 6 hours (Third, Sixth, Ninth, Eleventh/
Vespers, Twelfth/Compline) and the Midnight Office, then the Theotokia weekly cycle as Phase 2 (per
...
## SESSION 2026-08-18 continued -- Coptic Agpeya: Twelfth Hour (Compline) built -- ALL SEVEN HOURS COMPLETE

Sixth and final hour checkpoint, closing out the "hours first" half of the Agpeya rebuild. Source
(O'Leary 1911, pp. 106-109) had a real gap between this project's two uploaded PDF halves -- the
first file cut off mid-sentence partway through this hour's final prayer, and the second file
jumped straight to the Theotokia section rather than continuing where the first left off. Josh
pasted the missing page (109) directly, which resolved the gap and completed the Metremhe in full.

**Content built -- the complete Twelfth Hour, O'Leary's items (1)-(11):**
- `components/coptic.json`: 5 new entries (`cop-tw-esaias-prayer`, `cop-tw-troparion`,
  `cop-tw-litany`, `cop-tw-second-troparion`, `cop-tw-concluding-prayer`), 39 total.
- **No-placeholder rule applied to a new pattern -- a litany:** item (5)'s litany abbreviates every
  intercession line after the first as "...intercede, &c." Resolved by writing "intercede for us
  sinners" out in full on every single line, per the rule.
- **A clean example of the rule's other half -- reuse, not re-transcription:** item (6) is O'Leary's
  own explicit cross-reference back to the *same* "Vouchsafe, O Lord" prayer already built for the
  Eleventh Hour ("(6) Vouchsafe, O Lord (page 104)."), confirmed by matching page numbers -- reused
  directly as `cop-eh-vouchsafe-prayer` in this hour's sequence rather than duplicating the
  component or leaving a citation.
- `cop-tw-esaias-prayer` (item 2) and `cop-tw-concluding-prayer` (item 11, Metremhe) are both given
  in full by O'Leary -- the Metremhe only because Josh's pasted page 109 completed the sentence the
  first uploaded file cut off mid-way through.
- `components/traditions/coptic/rubrics.json`: added the `coptic-twelfth-hour` rubric. Its psalm
  citation ends the same way the Ninth Hour's did -- a Vulgate split (146/147) matching the
  Hebrew-147 boundary already resolved corpus-wide in this session's DRB numbering fix, read here
  as the single whole Psalm 147. **No Gospel lection is cited by O'Leary for this hour** (the only
  one of the seven without one) -- confirmed by its absence in the source rather than assumed, and
  the rubric has no `lesson` field as a result.

**Code wiring:** `SHARED_OFFICE_NAVIGATOR_CONFIGS.coptic.options` extended with the Twelfth Hour
(all seven hours now selectable); `index.html` drawer status text updated to reflect the Agpeya's
hour-cycle being complete.

**Verification:** `node --check` clean. Both JSON files parse. Full sweep of
`components/coptic.json` for any remaining placeholder text in actual prayer content -- zero found.
`index.html` div-tag balance confirmed unchanged.

**ALL SEVEN HOURS OF THE COPTIC AGPEYA ARE NOW BUILT.** This closes the "hours first" half of the
governance decision made at the start of this rebuild. Remaining work, per that same decision:
the Midnight Office (a genuinely separate office, not one of the seven day-hours, still needs
sourcing/building), then Phase 2 -- the Theotokia weekly hymn cycle -- and resolving
...
## Session 2026-08-30 continued -- Julian/Gregorian Easter-reckoning investigation. Found and
## used the existing precedent (js/calendar-eastern-orthodox.js's eoMode), added the same pattern
## for COE. Julian remains the default everywhere; confirmed byte-identical to the pre-change
## engine. SEED_VERSION v192 -> v193.

Josh asked to look into the Julian/Gregorian Easter finding from the Layer 3 session, and flagged
that this project should already have multi-calendar support -- and separately, that recent
sessions have been getting caught off guard by how much of the repo already exists, rather than
investigating thoroughly before starting new work. Both points taken. Did a full repo sweep this
time before writing any code: `grep -rl` for "julian"/"gregorian" across the whole repo (not just
East Syriac files), which surfaced `js/byzantine-paschalion.js`, `js/calendar-eastern-orthodox.js`,
`documentation/CALENDAR_ENGINE_DOCUMENTATION.md`, and the settings/UI wiring in `js/office-ui.js`
and `index.html` before touching anything.

**Found the precedent Josh referenced.** `js/calendar-eastern-orthodox.js` already has a real,
shipped `eoMode` ('new_calendar' | 'old_calendar') threaded through its whole calendar-building
chain, with a persisted setting (`selectedEoMode` in `js/office-ui.js`), a `<select>` in
`index.html`'s Horologion settings panel, and save/load wiring. **Important nuance, not just
copy-paste:** EO's `eoMode` only changes which calendar FIXED feasts are dated on -- Pascha itself
is always computed via the Julian Paschalion either way (`buildCalendar` calls
`computeOrthodoxPascha(year)` with no mode argument). The COE finding is a different, larger axis:
the ACOTE diocesan calendar uses a genuinely different EASTER ALGORITHM (Gregorian, not just a
different fixed-feast calendar), which shifts every one of the six Easter-anchored COE seasons
(Sauma, Qyamta, Shlihe, Qayta, Eliya-Sliwa, Muse), not just a handful of fixed dates.

**Built the same class of solution, adapted to the real shape of this specific problem:**
- New `computeGregorianEaster(year)` in `js/calendar-east-syriac.js` -- the standard Anonymous
  Gregorian algorithm (Meeus, Ch.8), independent of the existing Julian `julianEasterDate()`.
- `getEaster(year, easterMode)` -- `easterMode` defaults to `'julian'` (unchanged default,
  preserves every prior call site with zero argument changes needed).
- `easterMode`/`options` threaded through the full call chain that was already established:
  `getLiturgicalYear(date, options)` -> `getSeason(gregorianDate, options)` ->
  `getDayClass(gregorianDate, options)` -> `getFixedCommemorationsForDate(date, seasonData,
  options)`, plus `getPreFastSundayFoldSchedule(gregorianDate, options)`. Every function's
  `options` parameter is optional and defaults to Julian, matching the eoMode precedent's own
  `options.eoMode || 'new_calendar'` pattern exactly.
- `js/office-ui.js`: new `selectedCoeEasterMode` global (default `'julian'`), new
  `selectCoeEasterMode(mode)` function mirroring `selectEoMode` structurally, wired into
  `saveSettings()`/settings restoration the same way `eoMode` is. All 10 call sites of
  `EastSyriacCalendar.getDayClass(currentDate)` inside `renderEastSyriac()` updated to pass
  `{ easterMode: selectedCoeEasterMode }`.
- `index.html`: new "Easter Reckoning" dropdown in the East Syriac settings panel, mirroring the
  Horologion calendar-mode dropdown's structure and styling, defaulting to Julian, with an inline
  note explaining the real-world practice split and what changing it affects.

**Verified with real care, given how much already-shipped, already-verified work depends on this
engine's date computations:**
- All 30 internal self-tests pass (20 original + 5 new Gregorian-Easter spot-checks + 5 new
  default-equals-explicit-Julian regression checks).
- `js/office-ui.js`, `js/calendar-east-syriac.js` pass `node --check`. `index.html` spot-checked
  for the new panel and correct id/function references.

**UPDATE, same session: Josh asked for the same rigor already established elsewhere in this
project -- specifically the 100-year sweep methodology used for the BCP Holy Day transfer fix
(`js/calendar-engine.js`, 2024-2123, "verified with a 100-year sweep (282 checks, 0 errors)",
per `AUDIT_GOVERNANCE_LEDGER.md`'s own prior entry). The original verification above (72 sample
dates across 2024-2035) fell well short of that standard. Redone properly:**

- **Daily-resolution regression sweep, 2024-2123 (100 years), every single day: 36,524 days
  checked** against the pre-change engine (cloned at the commit immediately before this session),
  comparing default mode, explicit `{easterMode:'julian'}`, and the untouched pre-change file's
  own output -- **zero mismatches across all three, for every day in the full 100-year range**,
  including the 2100 century boundary.
- **Structural invariants, both modes, all 36,524 days each:** zero season-assignment failures,
  zero crashes, zero NaN dates, Easter always falls on a Sunday, Easter always falls within its
  astronomically-valid range (Gregorian Mar.22-Apr.25; Julian/Orthodox Gregorian-equivalent
  Apr.4-May.8) -- in both modes, every single day, no exceptions.
- **External ground-truth cross-check** (this engine's algorithms had only ever been validated
  against 5 known years internally before this): confirmed 2037's Julian Easter independently via
  web search -- "the second earliest [Orthodox Easter] date, March 23 in the Julian Calendar,
  last occurred in 1953, and will next occur in 2037... equivalent to April 5 in the Gregorian
  Calendar" -- this engine computes April 5, 2037 exactly. Confirmed the 2100 century-offset
  transition externally ("by the year 2100, the difference will be 14 days," multiple independent
  sources) matches this engine's own already-correct century-aware Julian Day Number conversion
  used for Easter itself (`julianToGregorian()`, unaffected by the separate bug below). Cross-
  validated the Gregorian algorithm at 2000, 2001, 2100, and 2101 against independently-known
  values -- all matched. **One test-data error caught and corrected in the process, not a code
  bug:** an initial hand-typed "expected" value for 2123 (April 1) turned out to be wrong; a
  from-scratch independent Python implementation of the same standard algorithm confirmed the
  JS engine's own answer (April 11) was correct all along. Recorded here so the correction is on
  the record, not just silently fixed.
- **Pre-Fast fold-schedule N-range quantified across the full 100 years, both modes** (not just
  spot-checked, as the original v191/v192 work did for 2024-2035 only):
  - Julian (the shipped default): N falls outside Maclean's documented 4-8 range in **2 of 100
    years** (2037, 2105 -- both N=3). Both externally confirmed as genuinely, unusually early
    Julian Easters (2037 is on record as the earliest Julian Easter since 1953). This is a real,
    previously-undetected gap in the already-shipped `getPreFastSundayFoldSchedule` work --
    the original verification only checked 2024-2035 and missed it. The function's existing
    defensive fallback handles it without erroring, but Maclean's fold rule itself has no stated
    answer for N=3. Not fixed this session -- the function remains unwired into any render path,
    same status as before -- but the claim in the v191 entry that this range was "verified" should
    be read as "verified for 2024-2035," not exhaustively.
  - Gregorian: N falls outside the 4-8 range in **37 of 100 years**, including several years with
    N=2 (more compressed than the single N=3 case originally observed for 2026). This is not a
    rare edge case in Gregorian mode -- over a third of years would need the fold rule extended
    or the fallback accepted as routine, not exceptional, if Gregorian mode's fold schedule is
    ever wired into a live render path. Disclosed plainly, not minimized.

**A second, separate real bug found by this same rigor pass -- pre-existing, NOT introduced by
this session's Easter-reckoning work, unrelated to which easterMode is selected:** four fixed
dates in this file (`denkhaStart`'s Jan.19 anchor, `epiphanyGreg`, `nativityGreg`'s Jan.7, and
`transfigurationGreg`'s Aug.19) all hardcode a literal "+13 days" Julian-to-Gregorian offset
directly into a month/day literal, rather than using this same file's own century-aware
`julianToGregorian()` JDN conversion already used for Easter itself. The correct offset is +13
only for 1900-2099; it becomes +14 from 2100 onward (externally confirmed: "beginning March 14,
2100... the difference between the Julian and Gregorian calendars will increase to 14 days,"
matching the century table already implemented for this exact purpose in both
`js/calendar-eastern-orthodox.js` and `js/calendar-engine.js`). Quantified: of the 100 years
2024-2123, **24 fall after the 2100 boundary** where the hardcoded offset is wrong by construction;
because the code rounds to "the next Sunday on or after" the target date, most of those 24 years'
error is masked by the rounding (the wrong day-of-month still rounds to the correct following
Sunday), but **3 confirmed years (2110, 2116, 2121) produce a Denkha season start a full week
later than correct** -- a real, dateable, user-visible error for anyone using this app in those
years, in either Easter-reckoning mode. Not fixed this session -- out of scope for the
Easter-reckoning-mode work specifically, and deserves its own dedicated pass rather than a rushed
fix bundled into this one, consistent with how the Annunciation/Easter-collision bug was handled
(disclosed plainly, fixed in a dedicated follow-up). Logged here as a new, real, quantified
backlog item.

**Confirmed the Gregorian mode produces real, correct shifts:** `getPreFastSundayFoldSchedule`'s
"Sunday before the Fast" moves from Feb.22 (Julian) to **Feb.15** (Gregorian) for 2026 -- exactly
matching the ACOTE diocesan calendar's own "15. Beginning of Great Fast" entry that triggered this
whole investigation.

**Not decided, and deliberately not decided by me:** whether Gregorian should become the new
default, whether it's specific to just this one diocese or reflects a wider practice shift, and
whether other ACOE-tracked feasts/seasons need a similar per-item disclosure the way Nativity
already got. Julian remains the default pending Josh's direction, exactly as Old Calendar remains
un-defaulted for EO.
