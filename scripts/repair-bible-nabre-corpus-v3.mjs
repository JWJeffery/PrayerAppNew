...
const reportPath = process.env.NABRE_REPAIR_REPORT || `/tmp/prayerappnew_nabre_repair_v3_${stamp}.json`;
const backupDir = `/tmp/prayerappnew_nabre_repair_v3_backup_${stamp}`;
...
const report = {
  ...
  removedInvalidIntegratedNabreFields: 0,
...
function stripScaffold(text) {
  return String(text || '')
    .replace(/^(?:[IVXLCDM]+\.\s+)?Letter of Jeremiah\s+Chapter\s+6\s+-\s+/i, '')
    .replace(/^[IVXLCDM]+\.\s+[^.]{2,120}\s+Chapter\s+\d+\s+-\s+/i, '')
    .replace(/^Chapter\s+\d+\s+-\s+/i, '')
    .replace(/^True and False Shame\s+-\s+/i, '')
    .trim();
}
...
function extractVerses(html, chapter) {
  ...
  while ((m = re.exec(html))) {
    ...
    if (!close) continue;
    const text = stripScaffold(cleanHtml(close.inner));
...
function allowedFallbackText(book, chapter, verse, current) {
  ...
  if (c === 41 && v === 14) return stripScaffold(current);
  return null;
}
...
function integratedAction(bookJson, file, sourceMaps) {
  const id = norm(bookJson?.meta?.id);
  const name = norm(bookJson?.meta?.name || path.basename(file, '.json'));
  if (id === '1esd' || id === '2esd' || name === '1esdras' || name === '2esdras') return { kind: 'remove-invalid-nabre' };
  if (id === 'lje' || name === 'theletterofjeremiah' || name === 'letterofjeremiah') return { kind: 'map-source', source: sourceMaps.get('baruch'), chapterRemap: new Map([[1, 6]]) };
  for (const c of [bookJson?.meta?.name, bookJson?.meta?.id, path.basename(file, '.json')].filter(Boolean)) {
    for (const a of sourceAliases(c)) if (sourceMaps.has(a)) return { kind: 'map-source', source: sourceMaps.get(a), chapterRemap: null };
  }
  return null;
}
...
async function main() {
  ...
  for (const file of integratedDirs.flatMap(listJsonFiles)) {
    ...
    const action = integratedAction(json, file, sourceMaps);
    if (!action) { report.mappingFailures.push({ file: rel(file), book: json?.meta?.name || path.basename(file, '.json') }); continue; }

    if (action.kind === 'remove-invalid-nabre') {
      for (const ch of chaptersOf(json)) for (const verse of versesOf(ch)) {
        if (verse?.text && typeof verse.text === 'object' && typeof verse.text.NABRE === 'string') {
          delete verse.text.NABRE;
          changed += 1;
          report.removedInvalidIntegratedNabreFields += 1;
        }
      }
    } else {
      if (!action.source) { report.mappingFailures.push({ file: rel(file), book: json?.meta?.name || path.basename(file, '.json') }); continue; }
      for (const ch of chaptersOf(json)) {
        const c = chapterNum(ch);
        const sourceChapter = action.chapterRemap?.get(c) ?? c;
        const cm = action.source.map.get(sourceChapter);
        if (!cm) continue;
