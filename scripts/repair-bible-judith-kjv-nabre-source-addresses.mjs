...
const reportPath = process.env.JUDITH_KJV_NABRE_SOURCE_ADDRESS_REPAIR_REPORT || null;
const failures = [];
const stripped = [];
const inserted = [];
...
function vNo(v, i) { return num(v?.number ?? v?.num ?? v?.verse) ?? i + 1; }
...
const report = { repair: 'judith-kjv-nabre-source-addresses', status: failures.length ? 'failed' : 'repaired', bibleTextMutation: true, activePath, policy: 'data/bible/registry/judith-source-address-policy.json', strippedPrefixCount: stripped.length, insertedLaneCellCount: inserted.length, createdRowCount: createdRows.size, stripped, inserted, createdRows: [...createdRows], failures };
if (reportPath) fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`);
console.log(JSON.stringify(report, null, 2));
if (failures.length) { console.log('ALL FAILED'); console.log('NEXT: Judith KJV/NABRE repair failed. Review failures before audit.'); process.exitCode = 1; }
else { console.log('ALL PASSED'); console.log('NEXT: Run Judith policy-aware exact collation audit.'); }
